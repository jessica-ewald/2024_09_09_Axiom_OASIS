# Curve fitting functions extracted from FastBMD
  ...
  fitoneitem <- function(i) {
    ...
    if (keepExp2) {
      ...
      if (!inherits(Exp2, "try-error")) {
        ...
        res_temp <- data.frame(gene.id = gene.id, mod.name = mod.name,
                               b = b.i, c = c.i, d = d.i,
                               e = e.i, f = f.i, SDres = SDres.i,
                               AIC.model = AIC.i,
                               lof.p = lof.pval.i, ctrl.mod = ctrl)
        item.fitres <- rbind(item.fitres, res_temp)
      }
    }
...
FilterDRFit <- function(fitObj, lof.pval = 0.1, filt.var = "AIC.model") {
  ...
  fitres.filt <- fitres.filt[fitres.filt[ , .I[which.min(get(filt.var))], by = item.ind]$V1]
...
PerformBMDCalc <- function(fitObj, ncpus = 1, num.sds = 1, bmr.method = "sample.mean", log10.dose = FALSE)
{
...
scoresPOD <- function(dat, dose, log10.dose=FALSE, num.sds = 1, filt.var = "AIC.model") {
